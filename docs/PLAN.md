...
- `POST /recommend` — body `{merchant, amount, category?}`; runs the scoring engine against the caller's real wallet, falling back to the seeded demo wallet if nothing's mapped to a card product yet, then ElevenLabs; returns the audio + the full ranked breakdown as JSON for the "Why" screen
- `POST /conversation` — body `{message, purchase, history?}`; runs the same scoring engine, asks Gemini to phrase the ranking as a sentence, and discards any reply that names a figure the engine didn't compute (falling back to a plain computed sentence instead); returns `{reply, purchasePatch, generated, voice}`
- `POST /conversation/voice` — multipart audio upload (plus optional `store`/`amount`/`category` form fields); transcribes the clip with Gemini, then runs the transcript through the exact same reply pipeline as `/conversation`; returns the same shape plus `transcript` (what Gemini heard, for the chat UI's own bubble)
...
**M6 — Voice capture + extraction** ✅ **DONE** (landed on `/conversation`, not `/recommend` as originally sketched here)
- Backend: `POST /conversation/voice` (`app/routers/conversation.py`) accepts a multipart audio clip, transcribes it with a new `transcribe()` function in `app/services/gemini.py` (same retry/backoff as the existing `translate()`; no separate STT service), then hands the transcript to `_reply()` — the exact same grounded-reply pipeline typed messages use via `POST /conversation`, refactored out of that handler so there is one honesty-checked reply path, not two. Fixed a real bug surfaced while building this: `_reply()` could describe stale (pre-patch) context when a message both stated a purchase and asked for a recommendation from a blank start — fixed by re-translating against the new context after a patch is applied. Voice input does not cross-reference the Nessie merchant catalog for category validation as this entry originally planned — Gemini's own category guess is used as-is, same as typed messages (see §9).
- Frontend: real microphone recording in `mobile-app/src/app/conversation.tsx` via `expo-audio`'s `useAudioRecorder` (tap mic to start, tap again to stop and send — no mid-recording cancel), wired to a new `services.conversation.sendVoice()` (`contracts.ts`, `http-services.ts`) with a platform branch: a real `Blob` fetched from the recorder's `blob:` URL on web, React Native's `{uri, type, name}` convention on native. Verified working end-to-end on web (Chrome); not yet tested on a real native device or simulator. iOS/Android mic permission strings rely on Expo Go's own defaults, not custom `Info.plist`/`AndroidManifest` entries.

**M7 — Full recommend pipeline** ✅ **DONE** (interim: JSON body, not voice yet)
- Backend: `POST /recommend` now takes `{merchant, amount, category?}`, builds the caller's real wallet (`app/scoring/adapter.py`) — or falls back to a seeded demo wallet if nothing's mapped to a card product yet (M4) — and ranks it with the M5 engine. Voice input ended up landing on a separate endpoint (`POST /conversation/voice`, M6) rather than as a multipart variant of this one; `/recommend` stays JSON-only.
